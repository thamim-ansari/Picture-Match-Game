...
class MatchGame extends Component {
  ...
  onRest = () => {
    this.setState({score: 0, time: 60, isGameInProgress: true})
  }
  ...
  getFilteredMatchList = activeTabId => {
    const filteredMatchList = imagesList.filter(
  ...
  onMatchGame = id => {
    ...
    if (randomImage.id === id) {
      ...
      this.setState({isGameInProgress: false, time: 0})
    }
  }

  render() {
    const {score, time, randomImage, activeTabId, isGameInProgress} = this.state
    const filteredMatchList = this.getFilteredMatchList(activeTabId)
    return (
      <div className="app-container">
        ...
        {isGameInProgress ? (
          <div className="quiz-image-container">
            ...
            <ul className="match-images-container">
              {filteredMatchList.map(eachItem => (
                <MatchItem
                  key={eachItem.id}
                  imageDetails={eachItem}
                  onMatchGame={this.onMatchGame}
                />
              ))}
            </ul>
          </div>
